...
- # Doubts
  collapsed:: true
	- DONE See if the table already has the Branch option
		- if HAS  => fine,
		- If DON'T => see about the creation of the field for the branch]
	- DONE On the table were the values are recorded can the ID be repeated
		- Does the ID mean the ID of transaction or it means the id of the line?
	- DONE when the TPC berings the values does it rely on the ID to sort it, would multiple lines with the same ID impact on the Reading of the Table.
		- If it's made in a view could it already sum all the values with the same ID?
	- DONE Does the debts and the credits are saved in the table by another function or the data sent is already it?
- # Steps
  collapsed:: true
	- DONE Found out teh view for the *Pré-Editados*
	  logseq.order-list-type:: number
	- DONE Found out the controller.js for the view.
	  logseq.order-list-type:: number
	- DONE found out were is the table the data is stored.
	  logseq.order-list-type:: number
	- DONE Found out if the table can receive multiple rows with the Same transaction ID
	  logseq.order-list-type:: number
	- DONE Found out if the TPC brings it out via View or directly from the table.
	  logseq.order-list-type:: number
	- DONE Does this table is used anywhere else?
	  logseq.order-list-type:: number
	- DONE Does the function that bring out the values is used anywhere else?
	  logseq.order-list-type:: number
